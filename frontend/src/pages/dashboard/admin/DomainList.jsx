...
function DomainList() {
    ...
    const filteredDomains = useMemo(() => {
        return allDomains.filter(domain => {
            ...
            // 2. Status (isActive)
            const matchesStatus = filters.status === "ALL" || (filters.status === "ACTIVE" ? domain.isActive : !domain.isActive);
            // 3. Branch
            const matchesBranch = filters.branch === "ALL" || domain.applicableBranches.includes(filters.branch);
    ...
    const clearFilters = () => {
        setFilters({ search: "", status: "ALL", branch: "ALL" });
    };
                ...
                <div className="flex-shrink-0 bg-white shadow-sm border-b border-gray-200 z-20">
                    ...
                    <div className="mt-8 px-0 md:px-8 pb-2">
                        ...
                        <p className="text-sm text-gray-500 mb-6 px-6 md:px-0">
                            Manage domains, update applicable branches, and monitor status.
                        </p>
                        {/* Filter Bar Toggle for small screens */}
                                ...
                                {branchDropdownOpen && (
                                    <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                                        ...
                                        {allBranches.map(branch => (
                                            <button
                                                key={branch._id}
                                                className={`w-full text-left px-4 py-2 text-sm hover:bg-red-50 hover:text-red-700 ${filters.branch === branch._id ? 'font-bold text-red-600' : ''}`}
                                                onClick={() => { setFilters(f => ({ ...f, branch: branch._id })); setBranchDropdownOpen(false); }}
                                            >
                                                {branch.name}
                                            </button>
                                        ))}
                    ...
                    <div className="flex-1 mx-1">
                        ...
                        <div className="w-full flex justify-end">
                            <div className="font-bold text-red-600 text-sm mb-2 mr-1">
                                Showing {filteredDomains.length} result(s)
                            </div>
                        </div>
                                            ...
                                            <tr>
                                                <td colSpan="5" className="py-12 text-center text-gray-400">
                                                    <div className="flex flex-col items-center justify-center">
                                                        ...
                                                        <p>No domains found matching your filters.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        )}
